<?php
    ...
    if(isset($_SESSION['admin'])){
        if(isset($_GET['giatri']) && isset($_GET['loc_trong'])){
            $giatri = $_GET['giatri'];
            $loc_trong = $_GET['loc_trong'];
            $giatri = str_replace('\'','\\\'',$giatri);
            $loc_trong = str_replace('\'','\\\'',$loc_trong);
            ...
            if($giatri !=''){
                ...
                if($loc_trong==0){
                    $sql = "SELECT * FROM kytucxa where tinhtrangphong=$giatri and songuoihientai=songuoitoida order by toanha,phong asc";
                }
                if($loc_trong==1){
                    $sql = "SELECT * FROM kytucxa where tinhtrangphong=$giatri and songuoihientai<songuoitoida order by toanha,phong asc";
                }
